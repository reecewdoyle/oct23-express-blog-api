...
router.get("/findById/:id", async (request, response, next) => {
	...
	let result = await UserModel.findById(request.params.id).populate("author").exec();
...
router.post("/findOneQuery", async (request, response, next) => {
	...
	let result = await UserModel.findOne(request.body).populate("author").exec();
...
router.post("/findManyQuery", async (request, response, next) => {
	...
	let result = await UserModel.find(request.body).populate("author").exec();
